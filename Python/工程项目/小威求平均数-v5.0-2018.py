...
def calculateAllDataFildaverage(row_num, col_num, span = 60, ignore = 0):
    ...
    while block_index < ( 24 * 365): # how many hours in one year (2017 is ordinary year)
        # if start_row_num >= 525076:
        #     break
        # if start_row_num == 121083 and is_searched is False:
        #     # fill 0 pass the line
        #     block_index = block_index + 1
        #     is_searched = True
        #     current_hour_in_one_day = current_hour_in_one_day + 1
        #     continue
        ...
        for col_index in range(col_num): # from column A to column AJ, 36 columns totally
            if flag_data_time is False: # just used to fill first two columns
                if col_index == 0:
                    ...
                    if start_row_num >= 520000:
                        result_sheet.cell(row = block_index + 1, column = 1).value = str("2017/12/31")
                    else:
            ...
            if col_index == 8 or col_index == 11: # ignore the column of I and L
                ...
                continue
            # kernel code of calculating the average
            result_sheet.cell(row = block_index + 1, column = col_index + 1).value =\
...
def getValidLength(begin_row_num, span, day):
    ...
    const_hour = sheet.cell(row = begin_row_num + 0, column = 2).value.hour # NOYTE: CONST #
    for index in range(span):
        current_hour = sheet.cell(row = begin_row_num + index, column = 2).value.hour
        if current_hour != const_hour:
            break
...
def averageCol(sheet, begin_row_num, span, col_num):
    ...
    for index in range(span):
        ...
        if str(cell_value)[0:2] == "--": # '-' means NaN
            illegal_num = illegal_num + 1
            continue
        if str(cell_value)[0:1] == "-":
            data_sum = data_sum - float(str(cell_value)[1:]) # update data_sum
        else:
            data_sum = data_sum + float(cell_value) # update data_sum
    if span == illegal_num:
        return 0
    else:
...
def checkLostMinutes(row_num, ignore = 0): # BUGGER #
    start_time_minute = -1
    count = 0
    range_index = 1
    for index in range(row_num - ignore):
        date_minute = sheet.cell(row = index + 1 + 2, column = 2).value.minute
        if (date_minute < start_time_minute) and (date_minute + 60 - start_time_minute >=2) or (date_minute - start_time_minute >= 2):  # the next number less than forward number
            if date_minute < start_time_minute:
                count = count + (date_minute + 60 - start_time_minute - 1)
                print(" > Lost-minute No." + str(range_index) + " [ " + str(index + 1 + 1) + " - "\
             + str(index + 1 + 2) + " ]  lost numbers: " + str(date_minute + 60 - start_time_minute - 1))
            else:
                count = count + (date_minute - start_time_minute - 1)
                print(" > Lost-minute No." + str(range_index) + " [ " + str(index + 1 + 1) + " - "\
             + str(index + 1 + 2) + " ]  lost numbers: " + str(date_minute - start_time_minute - 1))
            range_index += 1
        start_time_minute = date_minute
    print(" > Warning: There are " + str(count) + " time-point(minute) has lost !"), print()
    return count
...
def checkLostHours(row_num, ignore = 0):
    ...
    for index in range(row_num - ignore):
        ...
        if (date_hour < start_time_hour) and (date_hour + 24 - start_time_hour >= 2) or (date_hour - start_time_hour >= 2):
            ...
            if date_hour < start_time_hour:
                count = count + (date_hour + 24 - start_time_hour - 1)
            else:
                count = count + (date_hour - start_time_hour - 1)
            range_index += 1
        start_time_hour = date_hour
    print(" > Warning: There are " + str(count * 60) + " time-point(hour) has lost !"), print()
    return (count * 60)
