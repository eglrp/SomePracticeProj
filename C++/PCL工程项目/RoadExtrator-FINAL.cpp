            ...
            for (size_t i = 1; i < _vecFPIndex.size() - 1; ++i) {
                ...
                if (it == std::end(in)) { /* 不是特征点，可以进行删除 */
                    ...
                    if (__calcDistanceOf2Points(_cloud->points[_vecFPIndex[i]], _cloud->points[_vecFPIndex[i + 1]]) < remove  ||
                            __calcDistanceOf2Points(_cloud->points[_vecFPIndex[i]], _cloud->points[_vecFPIndex[i - 1]]) < remove) {
                        if (__computeSlopeRatio(_cloud->points[_vecFPIndex[i]], _cloud->points[_vecFPIndex[i + 1]]) > 0.95f &&
                                __computeSlopeRatio(_cloud->points[_vecFPIndex[i - 1]], _cloud->points[_vecFPIndex[i]]) > 0.95f)
...
int openAscIIFile(std::string path, pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud) {
    ...
    while (getline(ifs, lineStr)) {
        ...
        pcl::PointXYZ point;
        point.x = std::stod(lineArray[0].substr(3));
        point.y = std::stod(lineArray[1].substr(4));
...
int main(void) {
    ...
    std::vector<std::string> file_abs_path;
    std::vector<std::string> file_name;
    getFileAbsolutePath("F:\\LJXK_Test_Data\\lines0", file_abs_path, file_name);

    /* 读取点云 */
    for (auto path : file_abs_path) {

        /* 定义点云 cloud 并写入点云数据 */
        std::cout << "SrcPath:  " << path << std::endl;
        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
        openAscIIFile(path, cloud);
        ///////////////////////////////////////////////////
        // openAscIIFile 可能实现的代码
        //for (auto v : vec){
        //  pcl::PointXYZ p;
        ...
        std::cout << "Finished! Find " << cloud_fp->points.size() << " points!\n";
        std::cout << "Using " << during << "s!" << std::endl;

        /* 输出结果到本地 */
        std::ofstream ofs1("C:/Users/Administrator/Desktop/5.txt");
        for (auto p : cloud->points) {
            ofs1 << std::setprecision(7) << p.x << " " << p.y << " " << p.z << std::endl;
        }
        ofs1.close();

        std::ofstream ofs2("C:/Users/Administrator/Desktop/6.txt");
        for (auto p : cloud_fp->points) {
            ofs2 << std::setprecision(7) << p.x << " " << p.y << " " << p.z << std::endl;
        }
        ofs2.close();

        /* 暂停程序 */
        int a; std::cout << "Press any key to continue:\n";
        std::cin >> a;
        if (a == 666)
            break;
        std::cout << "\n--------------------------------------------------------\n\n";
